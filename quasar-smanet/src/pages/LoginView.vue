...
// Tambahkan ref untuk mengakses form
...
const onSubmit = async () => {
  // Gunakan form.validate() untuk memastikan v-model terupdate dan rules divalidasi
  const formIsValid = await loginForm.value.validate()

  if (!formIsValid) {
    // Jika validasi form gagal (karena input kosong atau rules tidak terpenuhi)
    // Notifikasi sudah ditangani oleh rules Quasar, jadi cukup return
    return
  }

  // Tunggu siklus update DOM berikutnya untuk memberi kesempatan autofill sinkron
  await nextTick()
  ...
  try {
    ...
    // Kirimkan SATU objek yang berisi email dan password
...
/* Gaya untuk input yang diisi otomatis oleh browser */
/* Menggunakan pseudo-element ::-webkit-autofill */
...
.q-input :deep(input:-webkit-autofill:active) {
  -webkit-box-shadow: 0 0 0px 1000px rgba(255, 255, 255, 0.1) inset !important; /* Warna background transparan */
  -webkit-text-fill-color: #fff !important; /* Warna teks putih */
  border-color: rgba(255, 255, 255, 0.5) !important; /* Warna border transparan */
  /* Anda mungkin perlu sesuaikan warna background dan teks agar sesuai dengan desain Anda */
}
